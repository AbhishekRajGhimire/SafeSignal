...
describe('never invents emergency advice', () => {
  it('leaves rawAdvice to the feed and never assigns authored text to it', () => {
    ...
    for (const file of FILES) {
      ...
      // Separators are normalised because join() yields backslashes on
      // Windows, so the bare 'sources/scenario.ts' check silently missed
      // the exemption there and only ever failed on Windows machines.
      if (file.split(sep).join('/').includes('sources/scenario.ts')) continue
